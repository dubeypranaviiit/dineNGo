...
import { FiUsers, FiCalendar, FiPlusCircle, FiEdit, FiSearch, FiMenu, FiX, FiLogOut } from 'react-icons/fi';
...
const menuItems: MenuItem[] = [
    {
        ...
        submenu: ["View Staff List", "Add New Staff", "Staff Roles and Permissions", "Staff Performance Tracking"],
    },
    {
        title: "Booking Approval",
        icon: <FiCalendar className="w-5 h-5" />,
        submenu: ["Pending Reservations", "Approved Bookings", "Reservation Management", "Quick Approval Interface"],
    },
            ...
            >
                <div className="p-4 space-y-6">
                                ...
                                {item.submenu && item.submenu.length > 0 && expandedSubmenu === item.title && (
                                    <div className="pl-10 space-y-1">
                                        {item.submenu.map((subItem) => (
                                            <Link href={`/admin/${subItem.toLowerCase().replace(/ /g, '-')}`} key={subItem}>
                                                <div className="cursor-pointer w-full text-left p-2 rounded-md hover:bg-gray-800 transition-colors">
                                                    {subItem}
                                                </div>
                                            </Link>
                                        ))}
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-4 border-t border-gray-800">
                        <button className="flex items-center space-x-2 text-red-400 hover:text-red-500 transition-colors" aria-label="Logout">
